...
%
% MAT292, Fall 2018, Stinchcombe & Parsch
...
% solution.
%yy=(((sin(t)).^2)-1)./(2*cos(t)); Exact Solution

% Label your axes and include a legend.
f = @(t,y) y * tan(t) + sin(t);
t0=0;
y0=-0.5;
soln=ode45(f,[t0,pi],y0);
tt=linspace(0,pi,50);
yy=(((sin(tt)).^2)-1)./(2*cos(tt));
plot(tt, yy, soln.x, soln.y, 'x', 'MarkerSize',10, 'LineWidth', 2)
title('y vs t, numerical vs exact plots');
xlabel('t');
ylabel('y');
legend('Exact', 'Numerical','Location','Best');
...
% label your axes.
yex=(((sin(soln.x)).^2)-1)./(2*cos(soln.x));
err=abs(yex-soln.y);
disp(err)
fprintf('maximum error: %g \n', max(err));
semilogy(soln.x, err, 'LineWidth', 2);
xlabel('t');
ylabel('error');
%max error occers when t=1.5588, and it is greatest here due to the 
%difference between the exact value of y calculated and y approcimate
%that was calculated by ode45 being the largest here. In the graph
%of the orginal function, it seems to be an inflection point, which may
%explain the large error, as rapid change relative to the surrounding graph
%occurs here.
%% Exercise 4
%
...
err=abs(ya.y-yexac)
max(err)
plot(tt, yy, ya.x, ya.y, 'x', 'MarkerSize',10, 'LineWidth', 2)
title('y vs t, numerical vs exact plots');
xlabel('t');
ylabel('y');
legend('Exact', 'Numerical','Location','Best');
fprintf('max error equals: %g \n', max(err));
%% Exercise 5
%
...
t0=0;
y0=-1;
soluti=ode45(f,[0,10],y0)
plot(soluti.x,soluti.y)
title('y vs t');
xlabel('t');
ylabel('y');
...
ylabel('y')
title('y vs t')
%the values of y quickly approach infinity after the values of t pass 0.5,
%and the iode directional fields agree with this interpretation, clearly
%showing near vertical slopes for the particular solution and IVs used
%when approaching 0.5
...
phi=[0];   % we will keep a list with all the approximations
...
picard=ezplot(phi(N+1),[-pi/2,pi/2]);
set(picard,'Color','green');         % set the color of the graph to green

% 

hold on;
exact=ezplot(tan(t+pi/4),[-pi/2,pi/2]);
