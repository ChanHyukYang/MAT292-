...
xin=[1,1];
[ti,sol]=solvesystem_yangc153(0,4*pi,xin,0.05,f);
x1=(1/151)*exp(-ti/4).*(151*cos((sqrt(151)*ti)/4)-5*(sqrt(151))*sin((sqrt(151)*ti)/4));
x2=(1/151)*exp(-ti/4).*(17*sqrt(151)*sin((sqrt(151)*ti)/4)+(151).*cos((sqrt(151)*ti)/4));
plot(sol(1,:),sol(2,:))
hold on
plot(x1,x2)
title('Exact vs Heun')
...
% Label your axes and include a legend. t for para.
...
hold on
plot(sol(1,:),sol(2,:))
title('Huen vs IODE Euler')
xlabel('x2')
ylabel('x1')
legend('Euler','Huen','Location','best')
%The Euler's method implementaion seen on iode with a step size of 0.05
%reveals a tighter spiral behaving in a similar fashion to that displayed
%with the Huen plot. However, the circle is noticeably tighter and does not
%fully spiral inwards, but does so very slowly. This can be attributed to
%euler's method taking the slope value at the point as the slope until the
%next, while improved euler takes a more accurate, averaged sloipe between
%the two end points of a step given from euler's method, treating that as
%the slope. The derivative values can osciallte wildly inebtween values, so 
%any improvements/changes to the range of values taken can result is 
%drastic differences. This results in a faster, more accurtate spiralling 
%inwards for Hune when compared to euler's method.
...
%
% 4.1. |dx/dt = [2 1; 1 3] x|
%Unstable source. eigenvalues: 5+-root(5)/2 => real, distinct eigenvalues
%>0 always

% 4.2. |dx/dt = [-2 -1; -1 -3] x|
%Stable sink. eigenvalues: -5+-root(5)/2 => real, distanct eigenvalues <0
%always

% 4.3. |dx/dt = [-4 -6; 3 5] x|
%saddle-point. eigenvalues: 2,-1 => 1 positive, 1 negative eigenvalue

% 4.4. |dx/dt = [4  6; -3 -5] x|
%saddle-point.eigenvalues: 1,-2 => 1 positive, 1 negative eigenvalue

% 4.5. |dx/dt = [0  -1; 1 -1] x|
%stable, counter clockwise spiral.eigenvalues: -0.5+-i(root(3)/2), distinct
%complex eigenvalues with a negative real component

% 4.6. |dx/dt = [0  1; -1 1] x|
%unstable clockwise spiral.eigenvalues: 0.5+-i(root(3)/2), distinct
%complex eigenvalues with a positive real component

% 4.7. |dx/dt = [2  8; -1 -2] x|
%clockwise centers.eigenvalues: 2i,-2i, purely imaginary complex
%eigenvalues, eigenvectors help with direction

% 4.8. |dx/dt = [-2 -8; 1 2] x|
%counter clockwise centers.eigenvalues: -2i,2i, purely imaginary complex
%eigenvalues, eigenvectors help with direction

% 4.9. |dx/dt = [-8 5; -13 8] x|
%clockwise centers.eigenvalues: i,-i, purely imaginary complex
%eigenvalues, eigenvectors help with direction

% 4.10. |dx/dt = [8 -5; 13 -8] x|
%counter clockwise centers.eigenvalues: i,-1, purely imaginary complex
%eigenvalues, eigenvectors help with direction
